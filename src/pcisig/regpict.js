          ...
          if (b === f.lsb) {
            ...
            if (isRegister) {
              ...
              if (f.lsb === f.msb) {
                ...
                if (debug) {
                  console.log(
                    `bitnum-middle ${f.lsb} at x=${middleOf(
                      f.lsb
                    )} y=${cellTop - 4}`
                  );
                }
            ...
            if (
              f.hasOwnProperty("addClass") &&
              typeof f.addClass === "string"
            ) {
            ...
            if (f.lsb > visibleMSB || f.msb < visibleLSB) {
              ...
              if (
                ...
                gAddClass.push(
                  `regFieldExternal${bitLineCount < 2 ? "0" : "1"}`
                );
  ...
  document
    ...
    .forEach(fig => {
      ...
      fig.querySelectorAll("pre.json,div.json,span.json").forEach(pre => {
        try {
          mergeJSON(json, pre.textContent);
